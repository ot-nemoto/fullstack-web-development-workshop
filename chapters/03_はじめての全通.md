...
## 3-1 2つのVSCodeウィンドウによる開発

Chapter 2 でバックエンド用とフロントエンド用の DevContainer を起動しました。この章からは、**2つのVSCodeウィンドウを並べて**開発を進めます。

```
バックエンドウィンドウ（Backend DevContainer）
  → Django を起動 → localhost:8000 でAPIを提供し続ける

フロントエンドウィンドウ（Frontend DevContainer）
...
ファイルを保存すると、バックエンドウィンドウのターミナルにDjangoが自動で再読み込みしたログが流れます。
...
export default async function Home() {
  ...
  // backend はdocker-composeのサービス名。コンテナ間通信ではサービス名でアクセスする
...
sequenceDiagram
    ...
    Next->>Django: fetch('http://backend:8000/api/hello/')
