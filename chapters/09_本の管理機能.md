...
一覧ページにフィルターのようなインタラクティブな機能を追加するには、**Server ComponentとClient Componentを分離する**パターンを使います。

```
BooksPage（Server Component）
  ← データをサーバー側でfetchする
  └── BooksClient（Client Component）
        ← useStateでフィルターを管理する
        └── BookCard × N
```
...
`useState` はブラウザ側の機能のため、ファイル先頭に `'use client'` の宣言が必要です。Server Componentの中では直接使えませんが、データ取得（Server Component）とインタラクション（Client Component）を分離することで両方の利点を活かせます。

### 🛠️ BooksClientを実装する

`frontend/src/components/BooksClient.tsx` を新規作成します。
...
`setShowAvailableOnly` を呼ぶと `showAvailableOnly` が更新され、コンポーネントが再描画されます。

### 🛠️ books/page.tsx をBooksClientに切り替える
...
`getBooks()` で取得したデータを `BooksClient` に props として渡します。データ取得はサーバー側、フィルター操作はブラウザ側で行う構成です。

`http://localhost:3000/books/` にアクセスして、「貸出可能のみ表示」チェックボックスで絞り込みが動作することを確認しましょう。
...
登録と編集で同じフォームを使い回すために、`frontend/src/components/BookForm.tsx` を先に作ります。`useState` でタイトル・著者・ISBNなど各フィールドの入力値を管理します。
...
カテゴリのデータが不足している場合は、管理画面（`http://localhost:8000/admin/`）または curl でいくつか追加しておきましょう。
...
- Server ComponentとClient Componentを分離するパターンで、フィルター機能を実装した
- useState でフォームの入力値・フィルター状態を管理する方法を習得した
- フォームコンポーネントを登録と編集で共通化し、バリデーションを実装した
- Chapter 6のAPIと接続し、フルスタックで機能が動く状態を体験した

> **参考コード**：この章完了時点のコードは [v2.4 - 本の管理機能](https://github.com/ot-nemoto/fullstack-web-development-workshop-app/releases/tag/v2.4) で確認できます。手順通りに進めてコードが動かない場合、ここと見比べてみてください。
