...
「カテゴリ名を Book テーブルに直接コピーして書けばよいのでは？」と思うかもしれません。しかし、たとえば「プログラミング」というカテゴリ名を全行に直接書いてしまうと、後で名前を変えたいときに全行を更新しなければなりません。代わりに **カテゴリの ID だけを持ち、詳細は Category テーブルを「参照」** することで、Category テーブルの1行を変えるだけで済みます。これが外部キーを使う理由です。
...
- **本を削除したとき**：その本の貸出履歴も不要になるので、一緒に削除する → `CASCADE`（連鎖削除）
- **カテゴリを削除したとき**：カテゴリがなくなっても本自体は残したいので、カテゴリの欄を空欄にする → `SET_NULL`

| 値 | 挙動 |
|----|------|
| `CASCADE` | 参照先の削除に合わせてこのレコードも削除する |
| `SET_NULL` | 参照先が削除されたらこのカラムを NULL にする |
| `PROTECT` | 参照先が削除されないように保護する（参照している間は削除できない） |
...
「設計図を書く → 実際に工事する」の2段階で実行します。

| コマンド | 役割 | 建築に例えると |
|---------|------|-------------|
| `makemigrations` | モデルの変更を検出して「マイグレーションファイル」を生成する | 設計図を書く |
| `migrate` | マイグレーションファイルをDBに適用してテーブルを作成・変更する | 工事する |

2段階に分かれているのは、生成された**マイグレーションファイルをGitで管理できる**ためです。「いつ誰がどのようにテーブル定義を変えたか」を履歴として残せます。チームで開発しているとき、自分のPCで `makemigrations` したファイルをコミットしておけば、他のメンバーは `migrate` を実行するだけで同じテーブル構成になります。
...
`filter()` の引数では `__`（ダブルアンダースコア）を使って「SQL の WHERE 句」のような絞り込み条件を Python らしく書けます。`フィールド名__条件名=値` の形式です。

| 記法 | 意味 | 例 |
|------|------|-----|
| `__contains` | 部分一致 | `title__contains="Python"` |
| `__exact` | 完全一致（省略可） | `status__exact="active"` |
| `__gte` | 以上（greater than or equal） | `available_count__gte=1` |
| `__lte` | 以下（less than or equal） | `available_count__lte=3` |
...
> **参考コード**：この章完了時点のコードは [v2.0 - データベース設計とDjangoモデル](https://github.com/ot-nemoto/fullstack-web-development-workshop-app/releases/tag/v2.0) で確認できます。手順通りに進めてコードが動かない場合、ここと見比べてみてください。

### 🛠️ 変更をコミットしてpushする

```bash
git add .
git commit -m "Chapter 5: データベース設計とDjangoモデル"
git push
```
