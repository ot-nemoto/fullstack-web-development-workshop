...
```bash
git clone --branch v0.9 --depth 1 https://github.com/ot-nemoto/fullstack-web-development-workshop-app.git
cd fullstack-web-development-workshop-app
```

`--branch v0.9` は取得するバージョンの指定、`--depth 1` はコミット履歴を1件だけ取得してダウンロード量を減らす指定です。

> **補足**：ここでの `git clone` はファイルを取得するためだけに使います。gitの初期設定・GitHub認証・自分のリポジトリへの接続はDevContainerを起動した後に行います。
