...
| ブランチ | 役割 |
|---------|------|
| `main` | 安定したコード（本番に近い状態） |
| `develop` | 開発中のコードを統合するブランチ |
| `feature/xxx` | 機能ごとの開発ブランチ |

新しい機能を作るときは `feature/` ブランチを切り、`develop` を経由して `main` にマージします。この流れはChapter 13で実際に使います。
...
Dockerより前から**仮想マシン**（VM）という技術がありました。VMはOS丸ごとを仮想化するため、起動に数分かかり容量も数GBになります。

Dockerの**コンテナ**はアプリの実行に必要な部分だけを分離します。ホストOSのカーネルを共有するため、軽量で起動が数秒で完了します。
...
**DevContainer**はVSCodeの機能で、Dockerコンテナの中で開発を行う仕組みです。`.devcontainer/devcontainer.json` という設定ファイルに「このプロジェクトの開発環境」を定義しておくと、VSCodeが自動的にコンテナを起動して接続します。
...
```bash
git clone https://github.com/ot-nemoto/library-system.git
cd library-system
```

### 🛠️ DevContainerを起動する

1. VSCodeで **ファイル → フォルダーを開く** から `library-system` フォルダを開く
2. 右下に「コンテナで再度開く」という通知が表示されたらクリックする
   - 表示されない場合は `Ctrl+Shift+P`（`Cmd+Shift+P`）でコマンドパレットを開き、`Dev Containers: Reopen in Container` を実行する
3. 初回はコンテナのビルドが始まります。完了まで数分かかります
4. 左下のステータスバーに `Dev Container: 図書館貸出システム` と表示されれば起動完了

コンテナが起動するとVSCodeがコンテナ内に接続した状態になります。以降のターミナル操作はすべてLinuxコンテナの中で実行されます。
...
`backend/` と `frontend/` が並んでいるモノレポ構成です。1つのVSCodeウィンドウで両方を触れます。
...
```yaml
services:
  app:                              # アプリサービス（Python + Node.js）
    build:
      context: .
      dockerfile: .devcontainer/Dockerfile
    volumes:
      - .:/workspace:cached         # ローカルのファイルをコンテナ内の/workspaceに共有する
...
2つのサービスがあります。`app` サービスがDevContainerとして使われる開発用コンテナで、`db` サービスがMySQLです。`app` コンテナから `db:3306` というホスト名でMySQLに接続できます（Chapter 5で実際に設定します）。
