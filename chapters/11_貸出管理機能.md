...
| `GET /api/loans/my/` | GET | 自分の貸出履歴を取得する |
...
class LoanViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if book.available_count <= 0:
            raise serializers.ValidationError({'book': 'この本は現在貸出できません'})
        ...
        if already_loaned:
            raise serializers.ValidationError({'book': 'この本はすでに借りています'})
...
上記の `perform_create` 内で `book.available_count <= 0` の条件がチェックです。`ValidationError` を発生させると、DRFが自動的に HTTP 400 レスポンスを返します。

```python
from rest_framework import serializers as drf_serializers
# ※ viewsファイルでserializersをimportするにはエイリアスで区別する
...
同じく `perform_create` 内の `already_loaned` チェックが対応します。`Loan.objects.filter(...).exists()` で「同一ユーザー・同一本・ステータス:active」のレコードが存在するかを確認します。
                            ...
                            return (
                                ...
                                >
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="font-medium">本ID: {loan.book}</p>
