...
> **この章が難しいと感じたら**：テストを書く意義は、実際に開発を続ける中で徐々に実感できるものです。概念が理解しきれなくても、コードをコピーして先に進んで構いません。開発経験を積んでから読み返すと、より深く理解できます。
...
> **補足**：インストール後に `npm audit` の警告が表示される場合があります。これは開発用パッケージに関するものであり、本番環境には影響しないため無視して構いません。
...
> **補足**：初回実行時は Next.js のコンパイルが走るため、40〜50 秒ほどかかります。2回目以降はキャッシュが効いて速くなります。
...
> **参考コード**：この章完了時点のコードは [v3.0 - テスト](https://github.com/ot-nemoto/fullstack-web-development-workshop-app/releases/tag/v3.0) で確認できます。手順通りに進めてコードが動かない場合、ここと見比べてみてください。

### 🛠️ 変更をコミットしてpushする

```bash
git add .
git commit -m "Chapter 13: テスト"
git push
```
