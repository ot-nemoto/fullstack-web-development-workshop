...
**リモートリポジトリ**はGitHubなどのサーバー上にあるリポジトリです。ローカルの変更を `push` で送信し、他のメンバーの変更を `pull` で取得します。

GitHubを使うには名前・メールアドレスの設定とGitHub認証が必要です。本書ではDevContainerを起動した後（2-5）にまとめて行います。DevContainerにはgitとGitHub CLI（`gh`）がはじめから入っており、認証情報はコンテナを再起動しても保持されます。
...
> **Windowsの方へ**：Docker Desktop の初回起動時に Windows ファイアウォールの許可ダイアログが表示される場合があります。「許可」を選択してください。許可しないとコンテナ間の通信やブラウザからのアクセスが機能しません。
...
本書では1つのDevContainerの中にPython 3.12とNode.js 22の**両方**を用意します。バックエンド（Django）もフロントエンド（Next.js）も同じコンテナ内のターミナルで操作します。ターミナルを2つ開き、それぞれでDjangoとNext.jsを起動して開発します（2-1で確認した「ターミナルを複数開く」の使いどころがここです）。

```
VSCodeウィンドウ
└── DevContainer（Fullstack Dev）
    ├── Python 3.12 環境
    ├── Node.js 22 環境
    ├── Django / Pythonライブラリ一式
    ├── Next.js / Node.jsパッケージ一式
    └── プロジェクトのコード（/workspace）
```
...
```
https://github.com/ot-nemoto/fullstack-web-development-workshop-app/releases/tag/v0.9
```

ページ下部の **Assets** セクションから **Source code (zip)** をクリックしてダウンロードします。

ダウンロードしたZIPファイルを解凍すると `fullstack-web-development-workshop-app-v0.9` というフォルダができます。このフォルダの名前を `fullstack-web-development-workshop-app` に変更します。

> **補足**：gitの初期設定・GitHub認証・自分のリポジトリへの接続はDevContainerを起動した後に行います。
...
3. 初回はコンテナのビルドが始まります。Python 3.12 と Node.js 22 のインストールが走るため、**完了まで10〜20分ほどかかります**
4. 左下のステータスバーに `Dev Container: Fullstack Dev` と表示されれば起動完了

コンテナが起動するとVSCodeがコンテナ内に接続した状態になります。ターミナルを開くと `/workspace` ディレクトリから始まり、Python も Node.js も使える環境です。
...
├── .devcontainer/
│   ├── devcontainer.json   ← DevContainer設定（使うFeature・起動後コマンド等）
...
`backend/` と `frontend/` が並んでいるモノレポ構成です。

### docker-compose.ymlの読み方

`.devcontainer/docker-compose.yml` を開いてみましょう。

```yaml
services:
  app:                              # 開発コンテナ（Python + Node.js 両方入り）
    image: mcr.microsoft.com/devcontainers/base:ubuntu
    volumes:
      - ..:/workspace:cached        # リポジトリ全体をコンテナ内の /workspace に共有する
      - gh-config:/root/.config/gh  # GitHub CLI の認証情報を永続化する
    ports:
      - "8000:8000"                 # ホストの8000番ポートをコンテナに繋ぐ（Django用）
      - "3000:3000"                 # ホストの3000番ポートをコンテナに繋ぐ（Next.js用）
...
2つのサービスがあります。`app` サービスがPythonとNode.jsを含む開発コンテナで、`db` サービスがMySQLです。`app` コンテナから `db:3306` というホスト名でMySQLに接続できます（Chapter 5で実際に設定します）。

`volumes` の設定（`- ..:/workspace:cached`）によって、ローカルで編集したファイルがリアルタイムでコンテナ内に反映されます。コンテナ内でコードを実行しながら、ローカルのVSCodeで編集できるのはこの仕組みのおかげです。
...
git add .
git commit -m "初期状態"
gh repo create fullstack-web-development-workshop-app --public --source . --push
```

GitHubに自分のリポジトリが作成され、コードがpushされます。以降は `git push` で自分のリポジトリに変更を送れます。

> **参考コード**：この章完了時点のコードは [v0.9 - Initial Development Environment](https://github.com/ot-nemoto/fullstack-web-development-workshop-app/releases/tag/v0.9) で確認できます。手順通りに進めてコードが動かない場合、ここと見比べてみてください。
...
- DevContainerを起動し、プロジェクト構造と `.devcontainer/docker-compose.yml` を読めるようになった
- DevContainer内でGitを初期化し、GitHubに自分のリポジトリを作成してpushした
