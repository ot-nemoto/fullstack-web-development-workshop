...
DjangoとNext.jsをそれぞれ起動するため、ターミナルを2つ用意します。

1. `Ctrl+@`（`Ctrl+@`）でターミナルを開く
2. ターミナルパネル右上の **＋** ボタンをクリックして2つ目を追加する
3. 各ターミナルを右クリック → **名前の変更** で `backend` と `frontend` と名付けると管理しやすい
...
```
ターミナル（backend）  → backend/ ディレクトリで Django を起動
                         localhost:8000 でAPIを提供し続ける

ターミナル（frontend） → frontend/ ディレクトリで Next.js を起動
                         localhost:3000 でページを提供し続ける
```
...
`Starting development server at http://127.0.0.1:8000/` が表示されれば起動成功です。このターミナルはそのままにしておきます。
...
ファイルを保存すると、ターミナル（backend）にDjangoが自動で再読み込みしたログが流れます。
